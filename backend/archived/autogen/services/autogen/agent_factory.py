...
class AgentFactory:
    ...
    def _create_agent_system_message(self, agent_type: str, system_message: str) -> str:
        ...
        try:
            # Get dynamic prompt from markdown files
            dynamic_prompt = agent_prompt_loader.get_agent_prompt(agent_type)
            if dynamic_prompt:
                # Combine with any additional system message
                if system_message:
                    return f"{dynamic_prompt}\n\n{system_message}"
                return dynamic_prompt
        except Exception as e:
            logger.warning("Failed to load dynamic prompt, using fallback", 
                         agent_type=agent_type, error=str(e))
        
        # Fallback to generic message if dynamic loading fails
        return f"""You are a helpful assistant agent specialized in {agent_type} tasks. {system_message}
