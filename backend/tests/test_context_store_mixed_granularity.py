...
class TestMixedGranularityIntegration:
    """Integration tests for mixed-granularity context storage."""

    @pytest.mark.asyncio
    async def test_full_granularity_workflow(self):
        """Test complete mixed-granularity workflow."""
        # Mock all components for full granularity workflow test
        with patch('backend.app.services.context_store.ContextStore') as mock_context_store:
            with patch('backend.app.services.artifact_service.ArtifactService') as mock_artifact_service:
                with patch('backend.app.services.granularity_analyzer.GranularityAnalyzer') as mock_granularity_analyzer:
                    ...
                    mock_granularity_instance = Mock()
                    mock_granularity_analyzer.return_value = mock_granularity_instance

                    # Mock granularity workflow
                    granularity_workflow = {
                    ...
                    mock_granularity_instance.analyze_complexity.return_value = {"score": 0.6, "level": "medium"}
                    mock_artifact_instance.determine_granularity.return_value = {"strategy": "sectioned"}
                    mock_context_instance.store_artifact.return_value = {"stored": True}

                    # Execute full granularity workflow
                    from app.services.mixed_granularity_service import MixedGranularityService

                    granularity_service = MixedGranularityService()
                    result = await granularity_service.process_content(
                    ...
                    assert result["content_analyzed"] == True
                    assert result["artifacts_created"] > 0
                    assert result["context_injected"] == True

    def test_context_store_mixed_granularity_validation_criteria(self):
        """Test that all mixed-granularity validation criteria are met."""
