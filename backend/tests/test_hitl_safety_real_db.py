...
class TestHITLSafetyServiceRealDB:
    ...
    def hitl_service(self, db_manager):
        """Create HITL safety service instance with a real database session."""
        with db_manager.get_session() as session:
            yield HITLSafetyService(db_session=session)
...
class TestHITLIntegrationRealDB:
    ...
    async def test_full_hitl_workflow_real_db(self, db_manager):
        ...
        with db_manager.get_session() as session:
            service = HITLSafetyService(db_session=session)
            ...
            with db_manager.get_session() as inner_session:
                approval = inner_session.query(HitlAgentApprovalDB).filter_by(id=approval_id).first()
                assert approval is not None
    ...
    async def test_budget_limit_workflow_real_db(self, db_manager):
        ...
        with db_manager.get_session() as session:
            ...
            with db_manager.get_session() as inner_session:
                ...
                inner_session.add(budget)
                inner_session.commit()
                inner_session.refresh(budget)
                db_manager.track_test_record('agent_budget_controls', str(budget.id))
            ...
            with db_manager.get_session() as inner_session:
                updated_budget = inner_session.query(AgentBudgetControlDB).filter_by(
                    project_id=project.id,
                    agent_type="TEST_AGENT"
                ).first()
